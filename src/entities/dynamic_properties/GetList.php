...
class GetList extends BaseFlRecord
{
    ...
    {
        if(!isset($this->user_id) && isset($session['person']['id'])) {
            $session = Yii::$app->session;
            $this->user_id = $session['person']['id'];
        }
        if(!isset($this->pagination_page)) {
            $this->pagination_page = 1;
        }
        if(!isset($this->pagination_row_count)) {
            $this->pagination_row_count = 5;
        }
        $queryParams = $this->setQueryParams($this->entity_class_id, $this->pagination_page, $this->pagination_row_count);
...
//            $defaultData['#type'] = 'string';
     ...
     * @return array
     */
    private function setQueryParams(int $entity_class_id, int $pagination_page, int $pagination_row_count): array
    {
        $data = [
//            'user_id' => (int)$user_id,
            'action' => 'properties_list',
            'params' => 
                [
                    'props' => '',
                    'where' => "class_id = $entity_class_id",
                    'order' => 'name',
