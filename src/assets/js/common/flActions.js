(function (global, factory) {
    ...
    FL_ACTIONS.extend({
        ...
        /**
         * Get config
         * TODO-docs Создаём конфиг на базе дефолтного конфига. Добавляем в него полученные данные 
         * из внешних источников(FLA.config.current_local, FLA.config.current, FLA.config.entities[config.current.entity]).
         * Клонируем config в actionConfig, это необходимо что бы не передавать данные через ссылки.
         * Если конфиг с сервера не сохранён в кэше, то делаем запрос на сервер. В противном случае,
         * выполняем setConfig.
         * @returns {this}
         */
        getConfig: function() {
            ...
            let actionConfig = {};
            /**
             * TODO-config_cache Проверяем сохранён ли уже конфиг с сервера. Если да, то используем его. Не реализованно.
            ...
            if(typeof FLA.config.entities !== "undefined" && typeof FLA.config.entities[config.current.entity] !== "undefined") {
                console.log("существует " + config.current.entity);
        ...
        /**
         * Set config
         * TODO-docs Сохраняем в кэш конфига (FLA.config.entities), конфиг с сервера(response.config).
         * Функция кэша пока не реализованна, будет доделана после точного определения задач.
         * Генерируем текущий конфиг(current). В начале вносим в него данные из конфига с сервера, потом вносим
         * значения из локального конфига(current_local). Если значения конфига с сервера и локального совпадают,
         * то они переписываются локальными. Тоесть локальный конфиг приоритетней конфига с сервера.
         * @param {Object} config
         * @returns {void}
         */
        setConfig: function(config) {
            ...
            /**
             * TODO-config_cache Если конфиг с сервера ещё не сохранён, то сохраняем. Не реализованно.
             * Обязательно тестировать.
             */
        ...
        createJsonEditorWorkspace: function(appConf, config, workspace = null) {
            ...
            if(typeof appConf.template === "string") {
                //TODO-json-editor Использовать кастомный шаблон. Не протестированно.
        ...
        /**
         * Создать рабочую область и шаблон для редактора ACE
         * Метод проверяет указан ли атрибут шаблона в конфиге. Если да то клонирует его.
         * Если нет, то ипользует шаблон по умолчанию. Далее получает рабочую область.
         * Если переменная "workspace" равна "null", то используется параметр конфига
         * "init_elem". После получения "workspace", в него вставляется шаблон.
        ...
        createAceEditorWorkspace:function(appConf, config, workspace = null) {
            let template;
            if(typeof appConf.template === "string") {
                //TODO-ace-editor Использовать кастомный шаблон. Не протестированно.
                template = $(appConf.template).clone();
        ...
        /**
         * TODO-refactoring возможно имеет смысл объеденить с buildJsGridContextMenu.
         * @param response
         * @param appConf
         */
         ...
         * TODO-docs Определяем рабочее пространстово для приложения jsgrid, получаем шаблон
         * и вставляем шаблон в рабочее пространство. Если шаблон не указан в конфиге,
         * берём шаблон по умолчанию. Добавляем в шаблон атрибут id. Добавляем в шаблон
         * title если в конфиге приложения присутствует параметр title.
         * @param {Object} appConf
         * @param {Object} config
         * @param {Object} workspace
        ...
        /**
         * Универсальная обработка событий из приложений FL_ACTIONS. Получаем имя события, текущий конфиг приложения,
         * и данные приложения не обходимые для выполнения сценария. Если в текущем конфиге приложения есть настройки
         * указанного события, то из настроек берётся имя метода-обработчика, и вызывает его, передвая ему конфиг и
         * данные приложения.
         * @param {Object} event
         * @param {Object} config
         * @param {Object} args
        ...
        /**
         * Перезагрузка сущностей с классом "fl-action.reload-as". На странице отыскиваются тэги с классом "fl-action-reload-as".
         * Теги должны содержать все необходимые аттрибуты для инициализации сущности. Далее из входящих данных приложения,
         * генерируется "actionData" и вызывается метод "setLocalDataAction".
         * @param {Object} config
         * @param {Object} args
         */
...
//                        dataKey = key;
        ...
        /**
         * Расчёт - отображение пагинации в приложении jsGrid.
        ...
        buildJsGridPagingAppSetPajes: function(pageCount, template, additionalData, workspace) {
            ...
            let prevPage;
            let nextPage;
            //Отображать ли First ссылку, биндиться параметры пагинации и метод, для вызова страницы.
            ...
            //Отображать ли Prev ссылку, биндиться параметры пагинации и метод, для вызова страницы.
            if(additionalData.pagination_page > 1) {
        ...
        /**
         * TODO-docs Проверка данных для пагинации. 
         * 1. Проверяем параметр settings.paging в конфиге приложения.
         * 2. Проверяем наличие данных для пагинации в ответе приложения(response.additional_data).
        ...
        createForm: function(config) {
            ...
            if(typeof config.current.element_id === 'string') {
                ...
                console.error('сonfig.current.element - not found');
                return false;
            }
